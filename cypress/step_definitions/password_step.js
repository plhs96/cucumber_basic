...
And('Check eamil {string} and set password when message {string} is successfull', (email, message, datatable) => {
    ...
    if ((message + '').includes('We have sent you a link to reset your password')) {
        ...
        data.forEach(element => {
            ...
            cy.setPassword(newPassword, confirmPassword)
            if (message != '') {
...
Then('Change password and verrify result', (datatable) =>{
    datatable.hashes().forEach(element => {
        cy.changePassword(element.currentPassword, element.newPassword, element.confirmPassword)
        if (element.error!='') {
            cy.checkExist(element.error)
        } else {
            cy.url().should('include', '/login')
        }
    });
